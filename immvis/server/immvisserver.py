...
import filetype
from .utils.extutils import is_csv, is_excel, is_image, is_json
from .datasets.imgdataset import read_image_as_dataframe
from concurrent import futures
import grpc
from .generated.immvis_pb2 import *
from .generated.immvis_pb2_grpc import *
import asyncio

_SERVER_LOOP_DELAY = 60 * 60 * 24
ERROR_CODE_UNKNOWN_EXTENSION = 1
ERROR_CODE_CANNOT_OPEN_FILE = 2
RETURN_CODE_SUCCESS = 0

class ImmVisServer(ImmVisServicer):
    data_frame = None
    ...
    def OpenDatasetFile(self, request, context):
        ...
        return OpenDatasetFileResponse(responseCode=responseCode)
    ...
    def GetDatasetDimensions(self, request, context):
        ...
        for column in self.data_frame:
            yield DimensionInfo(name=column, type=str(types[column]))
    ...
    def GetDimensionDescriptiveStatistics(self, request, context):
        ...
        for feature_name in desc_stats.keys():
            ...
            yield Feature(name=feature_name, value=feature_value, type=feature_type)
    ...
    def GetDimensionData(self, request_iterator, context):
        for dimension in request_iterator:
            ...
            if dimension_name is "":
                dimension_data = DimensionData(name="empty", type="empty", data=[])
            else:
                dimension_series = self.data_frame[dimension_name]
                ...
                dimension_data = DimensionData(name=dimension_name, type=dimension_type, data=dimension_data)
    ...
    def GetOutlierMapping(self, request_iterator, context):
        ...
        return DimensionData(name=dimension_name, type=dimension_type, data=dimension_data)
    ...
    def GetKMeansCentroids(self, request, context):
        ...
        for cluster_center in kmeans.cluster_centers_:
            ...
            yield KMeansCentroid(type='float64', coordinates=coordinates)
    ...
    def GetDatasetValues(self, request, context):
        ...
        for index, row in enumerate(self.data_frame.values):
            ...
            yield DataRow(index=index, values=row_values_str_list)
    ...
    def GetCorrelationBetweenTwoDimensions(self, request, context):
        ...
        return FloatResult(result=correlation)
...
async def start_server():
    ...
    add_ImmVisServicer_to_server(ImmVisServer(), server)
    server.add_insecure_port('[::]:50051')
    server.start()
    try:
        while True:
            await asyncio.sleep(_SERVER_LOOP_DELAY)
